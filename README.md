...
> [!IMPORTANT]
> **2026-04-12 中文版更新说明**
> - 在输入前输入链里新增了“切题判断”层，当前私聊主链路变成 `摘要 -> 切题 -> 路由 -> 主题 -> 决策 -> 改写 -> 召回 -> 输出`，先判断用户这轮是在继续当前话题、切到新话题，还是追问刚刚回忆出的历史。
> - 前端 RAG 流程条和设置面板已经同步显示切题层结果，现在可以直接看到上一轮是“切题 / 继续当前话题 / 历史追问”。
> - 日期回顾链路补上了按日期浏览、分块摘要、上下文分区和缓存失效修复，重点解决“问某天发生了什么却被最近强话题带偏”的问题。
> - 多个前置小模型步骤（切题、主题、决策、时间摘要）现在都会阻止 malformed / truncated 结果进入缓存；切题层一旦失败，会立即中止本轮并返回报错，而不是偷偷回退成“继续当前话题”。
> - README 已移除把 `vectra` 写成当前实时链路一部分的旧描述。当前默认实时记忆链路以 `Qdrant + SQLite 正文/元数据` 为主，不再把旧的本地 `vectra` 方案写成现行架构说明。
